...
public class Tokenizer {
	...
	//end token list:
	private static List<String> tokensList = new LinkedList<String>();
	//CORE language list
        	...
        	while(x < temp.length()) { //while start of token is not at end of line
        		...
        		if(Character.isLowerCase(currentChar)) { //char is lowercase - must be one of reservedWords
        			...
        			if((y<temp.length())&&searchArray(specialSymbols, Character.toString(temp.charAt(y)))) { //if the symbol has a symbol following it
        				if(searchArray(specialSymbols, temp.substring(x,y))) { //if the 2 symbols next to eachother are in specialSymbols
        					y++;
    ...
    public static void main(String[] args) throws IOException {
        ...
        List<String> inputProgram = new LinkedList<String>();
        //read in the file
        inputProgram = readInProgram(file);
        //tokenize the file
        Tokenizing(inputProgram);
        //for(String tokens:tokensList) {
        	//System.out.println(tokens);
        //}
        //convert the tokens to coressponding numbers
        List<String> tokenNumList = tokenToNum(tokensList);
        for(String tokens:tokenNumList) {
        	System.out.println(tokens);
        }
    }
